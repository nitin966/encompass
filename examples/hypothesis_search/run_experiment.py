#!/usr/bin/env python3
"""Run the complete hypothesis search experiment.

This script:
1. Runs the base hypothesis agent (without EnCompass)
2. Runs the EnCompass hypothesis agent 
...
from test_hypothesis import run_tests
...
async def run_experiment(model: str = "qwen2.5:32b") -> dict:
    ...
    print("=" * 70)
    print("HYPOTHESIS SEARCH EXPERIMENT")
    ...
    base_agent_lines = len((base_dir / "base_hypothesis_agent.py").read_text().splitlines())
    encompass_agent_lines = len((base_dir / "encompass_hypothesis_agent.py").read_text().splitlines())
    ...
    try:
        from base_hypothesis_agent import run_hypothesis_search as run_base
    ...
    start_time = time.time()
    try:
        from encompass_hypothesis_agent import run_hypothesis_search as run_encompass
    ...
    # === TEST OUTPUTS ===
    print("\n" + "=" * 70)
    print("PHASE 3: Testing Generated Functions")
    print("=" * 70)
    ...
    print(f"{'Tasks Solved':<30} {str(base_solved):<15} {str(enc_solved):<15}")
...
def generate_report(results: dict) -> str:
    ...
    return f"""# Hypothesis Search Experiment Report
...
| Tasks Solved | {comp['tasks_solved']['base_agent']} | {comp['tasks_solved']['encompass_agent']} | - |
...
- Manual `HypothesisState` enum
- Explicit state tracking with dataclasses
- Manual retry and refinement loops
- Complex control flow

**EnCompass agent simplicity:**
- `branchpoint()` for hypothesis exploration
- `record_score()` for accuracy feedback
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run Hypothesis Search Experiment")
